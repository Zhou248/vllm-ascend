...
HIGH_PRECISION_WINDOW_SIZE = 32
ATTENTION_SINK_SIZE = 32
...
class AscendAttentionBackendImpl(AttentionImpl):
        ...
        if using_paged_attention(num_tokens, vllm_config):
            ...
            with torch.npu.stream(update_stream):
                ...
                for key, param, handle, event in zip(
                    ...
                    logger.info(f"latest_npt:{latest_npt}")
                    logger.info(f"{evicted_kv_seq_lens_buffer is not None and latest_cpu_seq_lens is not None}")
                    if evicted_kv_seq_lens_buffer is not None and latest_cpu_seq_lens is not None:
                        ...
                        logger.info(f"prefill_len_buffer is not None:::::{prefill_len_buffer is not None}")
                        if prefill_len_buffer is not None:
                            logger.info(f"latest_npt:{latest_npt}")
                            if latest_npt is not None:
                                npt_reqs = latest_npt.numel()
                                logger.info(f"prefill_len_buffer更新前:{prefill_len_buffer}")
                                prefill_len_buffer[:npt_reqs].copy_(latest_npt.to(torch.int32)*0+1, non_blocking=True)
                                logger.info(f"prefill_len_buffer更新后:{prefill_len_buffer}")
                    ...
                    # [LAYER-1 PROBE] 图外验证：replay 前同步 update_stream，
                    # 确认 prefill_len_buffer 已被上面的 copy_(*0) 刷成全 0。
                    # 这是图外 Python 控制流，每次 replay 都执行（不在图内）。
                    if prefill_len_buffer is not None and latest_cpu_seq_lens is not None:
                        update_stream.synchronize()
                        _probe = prefill_len_buffer[:num_reqs].to("cpu")
                        if int(_probe.abs().sum()) == 0:
                            raise RuntimeError(
                                f"[LAYER-1 PROBE] prefill_len_buffer 被 update 刷成全 0 "
                                f"→ update_graph_params 生效确认。npt_reqs={num_reqs}, "
                                f"buffer[:npt_reqs]={_probe.tolist()}"
                            )
    ...
    def forward(
        ...
        if key is not None and value is not None:
            ...
            if not is_decode:
                key, value = self._quantize_kv_to_fp4_sink(
    ...
    def _quantize_evicted_kv_slots(
        ...
        # clamp 收束负数/越界位置防止索引越界，写回时再用 mask 屏蔽
    ...
    def _quantize_window_kv_slots_eager(
        ...
        for req_idx in range(num_decodes):
            ...
            # 触发判定：decode 区累积满一整窗 W 个 token 才触发。窗口基准定在
            # decode 起点（prefill_len）而非序列绝对起点，避免切到 prefill 的 FP4
            # 历史区造成二次量化。
    ...
    def _quantize_window_kv_slots_graph(
        ...
        # [LAYER-2 PROBE] 图内纯 tensor 探针：取消 mask 与 torch.where 条件写回。
        # 用 1/prefill_t 作为污染系数，广播到拍平的窗口维度 (num_decodes*W,)：
        #   - prefill_len != 0（update 未生效/正常）→ 系数为有限小数 → 写回近似原值，无副作用
        #   - prefill_len == 0（update *0 生效）→ 1/0 = inf → k/v 被 inf 污染 → 无条件写回 cache
        #     → 下游 attention 出现 NaN/inf，输出必然异常（图内 device 端无 Python raise 通道，
        #       故以"输出 NaN"作为 update 生效的可观测信号）。
        # 每个请求的 W 个窗口 slot 共享同一 prefill_t 值，故 repeat_interleave(W) 对齐 flat 维度。
        # dtype 必须与 KV cache (bfloat16) 对齐，否则 scatter 的 aclnnIndexPutImpl 会报 dtype 不匹配。
        probe_factor = (1.0 / prefill_t).repeat_interleave(W).to(k_quantized.dtype)  # (num_decodes*W,)
        updated_k = (k_quantized * probe_factor.unsqueeze(-1).unsqueeze(-1)).to(flat_key.dtype)
        updated_v = (v_quantized * probe_factor.unsqueeze(-1).unsqueeze(-1)).to(flat_value.dtype)
