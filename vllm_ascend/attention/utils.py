...
class AscendCommonAttentionMetadata(CommonAttentionMetadata):
    ...
    # CPU tensor of prompt token counts per request (fixed at request add time).
    # E.g., tensor([100, 200, 50]) means req0's prompt has 100 tokens. This is
    # the per-request prefill length / decode starting point, used to anchor the
    # high-precision window on the decode region instead of the absolute origin.
